...
<table width="350" border="0" cellpadding="20" cellspacing="1" bgcolor="#CCCCCC">
	<tr>
		...
		プロフィールの変更を行います。<br>
		ID、とパスワードを入力し「OK」ボタンをクリックして下さい。<br>
		<form action="/profile/form" method="post">
		ユーザーID：<input type="text" name="id" value="<?php if(isset($returnData)) echo $returnData['id']; ?>"><br>
		パスワード：<input type="password" name="ps" value="<?php if(isset($returnData)) echo $returnData['ps']; ?>"><br>
